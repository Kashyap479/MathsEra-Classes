...
function renderEditor(){
 ...
 if(editing){
   for(const f of fs){
     if(f[2]==="media"||f[2]==="media-mobile"||f[2]==="select-placement"||f[2]==="select-auto")continue;
...
async function getFormObject(fs){
 ...
 for(const f of fs){
   ...
   const v=el.value.trim();if(f[3]&&!v){editorMsg(f[1]+" is required.",false);return null;}obj[f[0]]=v;
 }
 ...
 if(current==="posters"){
   ...
   obj.description=JSON.stringify({
     ...
     order:Number($("f_display_order")?.value)||1,
